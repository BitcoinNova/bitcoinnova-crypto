...
namespace Common
{
    ...
    uint64_t fromHex(
        ...
        void *data,
        uint64_t bufferSize); // Assigns values of hex 'text' to buffer 'data' up to 'bufferSize', returns actual data
                              // size, throws on error
    bool fromHex(
        const std::string &text,
        void *data,
        uint64_t bufferSize,
        uint64_t &size); // Assigns values of hex 'text' to buffer 'data' up to 'bufferSize', assigns actual data size
                         // to 'size', returns false on error, does not throw
    ...
    template<typename T> bool podFromHex(const std::string &text, T &val)
    {
        uint64_t outSize;
        return fromHex(text, &val, sizeof(val), outSize) && outSize == sizeof(val);
    }
    std::string
        toHex(const void *data, uint64_t size); // Returns hex representation of ('data', 'size'), does not throw
    ...
    template<typename T> T fromString(const std::string &text)
    { // Throws on error
        T value;
    ...
    template<typename T> std::vector<T> fromDelimitedString(const std::string &source, char delimiter)
    { // Throws on error
        std::vector<T> data;
    ...
    template<typename T> bool fromDelimitedString(const std::string &source, char delimiter, std::vector<T> &data)
    { // Does not throw
        for (uint64_t offset = 0; offset != source.size();)
        {
            T value;
    ...
    template<typename T> void toString(const T &value, std::string &text)
    { // Does not throw
        std::ostringstream stream;
        stream << value;
        text += stream.str();
